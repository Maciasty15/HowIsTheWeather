...
# Stałe API
OPENMETO_BASE_URL = os.getenv("WEATHERAPI_API_BASE_URL", "https://air-quality-api.open-meteo.com/")
                ...
                if datetime_str:
                    ...
                    for i, time in enumerate(times):
                        if time >= datetime_str:
                            time_index = i
                            break
                    if time_index is None and times:  # If no future time found, use the last available
        ...
        if response.status_code == 200:
            ...
            if 'hourly' in data and 'time' in data['hourly']:
                ...
                if time_index is not None:
                    ...
                    current_carbon_monoxide = hourly.get('carbon_monoxide', [0])[time_index] or 0
                    current_nitrogen_dioxide = hourly.get('nitrogen_dioxide', [0])[time_index] or 0
                    
                    print(f"Values: PM10={current_pm10}, PM2.5={current_pm2_5}, CO={current_carbon_monoxide}, NO2={current_nitrogen_dioxide}")
